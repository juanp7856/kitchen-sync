...
const AuthScreen: React.FC<AuthScreenProps> = ({ onEntry }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Avatar selection is handled by the emoji picker below
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    savedProfiles[normalizedEmail] = { name, avatar };
    localStorage.setItem('kitchen-sync-profiles', JSON.stringify(savedProfiles));

    // onEntry ya no incluye role - el rol se deriva de useHostManager
    onEntry({ name, avatar, email: normalizedEmail });
  };
  ...
  const handleEmailSubmit = (e: React.FormEvent) => {
    ...
    const savedProfiles = JSON.parse(localStorage.getItem('kitchen-sync-profiles') || '{}');
    const userProfile = savedProfiles[normalizedEmail];

    if (userProfile) {
      // Returning user - skip profile step, enter directly
      onEntry({ 
      ...
      <div className="w-full max-w-md bg-black/40 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-8 shadow-2xl animate-in fade-in zoom-in duration-300">
        ...
        {step === 'auth' ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full bg-white text-black font-black py-4 sm:py-5 rounded-2xl sm:rounded-3xl text-lg sm:text-xl hover:bg-kitchen-cool hover:text-white transition-all transform active:scale-95 shadow-xl"
            >
              ¡OÍDO COCINA!
            </button>
